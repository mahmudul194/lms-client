      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-7 bg-white rounded-3xl p-7 border border-slate-200 shadow-xs space-y-5">
          ...
          {upcomingClass ? (
            <div className="space-y-4">
              <h4 className="text-base sm:text-lg font-bold text-slate-900 leading-snug">{upcomingClass.title}</h4>
              <div className="text-sm text-slate-700 space-y-2">
                <p>Instructor: <strong className="text-slate-900">{upcomingClass.instructor}</strong></p>
                <p>Time: <strong className="text-[#002b5b]">{upcomingClass.date} — {upcomingClass.time}</strong></p>
                <p className="font-semibold text-slate-600">Meeting ID: <strong className="text-slate-900">{upcomingClass.meetingId}</strong> • Passcode: <strong className="text-slate-900">{upcomingClass.passcode}</strong></p>
              </div>
              <div className="pt-2 flex items-center gap-3">
                <a href={upcomingClass.zoomLink} target="_blank" rel="noopener noreferrer" className="px-6 py-3 rounded-xl bg-[#0077b6] hover:bg-[#005a8c] text-white text-sm font-bold flex items-center gap-2 shadow-md hover:scale-105 transition-all">
            ...
            <p className="text-sm text-slate-500 py-4">No upcoming live classes scheduled at this time.</p>
          )}
        </div>
        ...
        <div className="lg:col-span-5 bg-white rounded-3xl p-7 border border-slate-200 shadow-xs space-y-5">
          <div className="flex items-center justify-between border-b border-slate-100 pb-4">
            ...
            <button onClick={onNavigateToResources} className="text-xs sm:text-sm text-[#0077b6] hover:underline font-bold cursor-pointer">
              View All
            </button>
          </div>
          ...
          <div className="space-y-3">
            {resources.length === 0 ? (
              <p className="text-sm text-slate-500 py-4">No course downloads available yet.</p>
            ) : (
              resources.slice(0, 3).map((res, i) => (
                <div key={i} className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center justify-between text-xs sm:text-sm">
