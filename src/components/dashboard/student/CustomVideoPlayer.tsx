...
import { extractYouTubeVideoId } from "@/hooks/useYouTubeController";
...
export default function CustomVideoPlayer({ videoUrl, title, onEnded }: CustomVideoPlayerProps) {
  ...
  }, [isYouTube, isPlaying, isMuted, volume, duration]);
  ...
  if (isYouTube && ytVideoId) {
    return (
      <div className="relative aspect-video bg-slate-950 overflow-hidden rounded-2xl shadow-xl border border-slate-800">
        <iframe src={`https://www.youtube-nocookie.com/embed/${ytVideoId}?rel=0&modestbranding=1&playsinline=1`} title={title} className="w-full h-full border-0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen />
      </div>
    );
  }
  ...
  return (
    <div ref={containerRef} onMouseMove={handleMouseMove} onContextMenu={(e) => e.preventDefault()} className="relative aspect-video bg-slate-950 overflow-hidden rounded-2xl group select-none font-sans">
      <video ref={videoRef} key={videoUrl} src={videoUrl || "https://cdn.plyr.io/static/demo/View_From_A_Blue_Moon_Trailer-720p.mp4"} onTimeUpdate={() => videoRef.current && setCurrentTime(videoRef.current.currentTime)} onLoadedMetadata={() => videoRef.current && setDuration(videoRef.current.duration)} onEnded={() => { setIsPlaying(false); onEnded?.(); }} onError={() => setIsPlaying(false)} onClick={togglePlay} className="w-full h-full object-cover cursor-pointer" playsInline preload="metadata" />

      {/* Top Header */}
      <div className={`absolute top-0 inset-x-0 p-4 bg-gradient-to-b from-black/85 via-black/30 to-transparent flex items-center justify-between text-white transition-opacity pointer-events-none ${showControls ? "opacity-100" : "opacity-0"}`}><span className="text-xs sm:text-sm font-bold text-white/95 drop-shadow-md truncate max-w-lg">{title}</span></div>

      {/* Big Play Button */}
      {!isPlaying && (
        <button onClick={togglePlay} className="absolute inset-0 m-auto w-18 h-18 rounded-full bg-gradient-to-br from-[#0077b6] to-[#002b5b] hover:from-[#005a8c] hover:to-[#001830] text-white flex items-center justify-center shadow-2xl backdrop-blur-md transition-all hover:scale-110 cursor-pointer border-2 border-white/80"><Play className="w-8 h-8 fill-white ml-1 text-white" /></button>
      )}
