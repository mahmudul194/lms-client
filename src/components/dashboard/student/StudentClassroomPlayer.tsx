        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white p-6 sm:p-7 rounded-3xl border border-slate-200/80 shadow-2xs space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
              <div><span className="text-xs font-black uppercase text-[#0077b6] tracking-wider">Active Lecture</span><h1 className="text-xl sm:text-2xl font-black text-slate-900 mt-0.5">{activeLesson.title}</h1><span className="text-sm font-semibold text-slate-500 font-mono mt-1 block">Duration: {activeLesson.duration}</span></div>
              <div className="flex items-center gap-2">
                <button onClick={() => prevLesson && handleSelectLesson(prevLesson)} disabled={!prevLesson} className={`px-4 py-2.5 rounded-2xl text-sm font-bold flex items-center gap-1.5 ${prevLesson ? "bg-slate-100 hover:bg-slate-200 text-slate-800 cursor-pointer" : "bg-slate-50 text-slate-400 cursor-not-allowed"}`}><ChevronLeft className="w-4 h-4" /> Previous</button>
                <button onClick={handleNextAndComplete} disabled={!nextLesson} className={`px-5 py-2.5 rounded-2xl text-sm font-bold flex items-center gap-1.5 shadow-xs ${nextLesson ? "bg-[#0077b6] hover:bg-[#002b5b] text-white cursor-pointer" : "bg-emerald-600 text-white cursor-default"}`}>{nextLesson ? <><span>Next & Complete</span><ChevronRight className="w-4 h-4" /></> : <><span>Course Finished</span><CheckCircle2 className="w-4 h-4" /></>}</button>
              </div>
            </div>
              ...
              <div className="pt-3 border-t border-slate-100 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {activeLesson.resources.map((r, i) => (
                    <div key={i} className="p-3.5 rounded-2xl bg-sky-50/70 border border-sky-200 flex items-center justify-between gap-3 text-sm">
                      <div className="truncate"><strong className="text-slate-900 block truncate font-bold">{r.name}</strong><span className="text-xs text-[#0077b6] font-mono font-medium">{r.size} • {r.type}</span></div>
                      <button className="p-2 rounded-xl bg-white hover:bg-sky-600 hover:text-white text-[#0077b6] shadow-2xs border border-sky-200 transition-all cursor-pointer"><Download className="w-4 h-4" /></button>
                    </div>
                  ))}
                </div>
              </div>
