...
export default function AdminRecordingsTab() {
  const [recordings, setRecordings] = useState<ClassRecordingSubmission[]>([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res = await liveSchedulesApi.getAllLiveSchedules({ limit: 50 });
        if (res.statusCode === 200 && res.data?.items) {
          const apiRecs: ClassRecordingSubmission[] = res.data.items.map((ls, idx) => ({
            id: ls.id,
            courseName: ls.batch?.name || "Professional BIM Detailing",
            batchCode: ls.batch?.code || "REV-8TH",
            classNo: String(idx + 1),
            topic: ls.title,
            instructorName: ls.mentor?.user?.name || "Lead Instructor",
            rawRecordingUrl: ls.meetingUrl,
            attachedModel: "Structural_Base_Model.rvt",
            submittedAt: ls.startTime ? new Date(ls.startTime).toLocaleDateString() : "Today",
            duration: "1h 45m",
            notes: ls.description || "Class live recording ready for student portal.",
            status: ls.status === "completed" ? "Uploaded to YouTube" : "Pending Admin Upload",
            youtubeUnlistedUrl: ls.platform === "youtube" ? ls.meetingUrl : undefined,
          }));
          setRecordings(apiRecs);
        }
      } catch {}
    })();
  }, []);
  ...
  const handlePublishYouTube = async (id: string) => {
    ...
    try {
      const { liveSchedulesApi } = await import("@/services/api/liveSchedulesApi");
      await liveSchedulesApi.updateLiveSchedule(id, { meetingUrl: url, status: "completed" });
    } catch {}
    setRecordings((prev) =>
    ...
    <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6 font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Review raw recordings from instructors and publish to YouTube</p>
        </div>
        <div className="flex items-center gap-2">
          {(["All", "Pending", "Uploaded"] as const).map((tab) => (
            <button key={tab} onClick={() => setFilter(tab)} className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition-all cursor-pointer ${filter === tab ? "bg-[#002b5b] text-white shadow-xs" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>
              {tab === "Pending" ? "Pending Upload" : tab === "Uploaded" ? "Published" : "All Submissions"}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filtered.length === 0 ? (
          <div className="p-8 text-center text-slate-500 font-semibold bg-slate-50 rounded-2xl border border-slate-200">
            No class recording submissions found in queue.
          </div>
        ) : (
          filtered.map((rec) => (
            <div key={rec.id} className="p-6 rounded-3xl border border-slate-200 bg-slate-50/70 hover:bg-white hover:shadow-md transition-all space-y-4">
              ...
              {activeModalId === rec.id && (
                <div className="p-4 rounded-2xl bg-white border-2 border-[#0077b6] space-y-3">
                  <label className="text-xs font-bold text-slate-800 block">Enter Video Embed Link (YouTube Unlisted / Vimeo):</label>
                  <div className="flex gap-2">
                    <input type="url" value={youtubeInput} onChange={(e) => setYoutubeInput(e.target.value)} placeholder="https://youtube.com/embed/..." className="flex-1 px-3 py-2 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-[#0077b6]" />
                    <button onClick={() => handlePublishYouTube(rec.id)} className="px-4 py-2 bg-[#0077b6] text-white font-bold text-xs rounded-xl hover:bg-[#005a8c]">Publish</button>
                  </div>
                </div>
              )}
            </div>
