...
import { Video, Copy, Check, Monitor, ExternalLink } from "lucide-react";
import { InstructorBatch } from "@/data/instructorMockData";

export default function InstructorBatchesTab({ batches }: { batches: InstructorBatch[] }) {
  ...
  const progressPercent = Math.round((currentBatch.completedClasses / currentBatch.totalClasses) * 100);
    ...
    <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-7 font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1">Monitor syllabus progress, student rosters, and launch live Zoom classes</p>
        </div>
      ...
      <div className="p-6 sm:p-7 rounded-3xl bg-[#001830] text-white space-y-5 shadow-xl border border-slate-800">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
          <div>
            ...
            <h4 className="text-lg sm:text-xl font-black text-white mt-0.5">{currentBatch.nextClassTopic}</h4>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs sm:text-sm">
          <div className="p-3.5 bg-white/5 rounded-2xl border border-white/10"><span className="text-slate-400 block text-xs">Meeting ID</span><strong className="text-base font-black text-white mt-0.5 block">872 9102 4819</strong></div>
          <div className="p-3.5 bg-white/5 rounded-2xl border border-white/10"><span className="text-slate-400 block text-xs">Host Passcode</span><strong className="text-base font-black text-white mt-0.5 block">BIM2026HOST</strong></div>
          <div className="p-3.5 bg-white/5 rounded-2xl border border-white/10"><span className="text-slate-400 block text-xs">Live Time</span><strong className="text-base font-black text-sky-300 mt-0.5 block">Tonight 9:00 PM</strong></div>
        </div>

        <div className="flex flex-col sm:flex-row items-center gap-3 pt-1">
          <a href="https://zoom.us/j/87291024819" target="_blank" rel="noopener noreferrer" className="w-full sm:flex-1 py-3.5 rounded-2xl bg-[#0077b6] hover:bg-[#005a8c] text-white font-black text-sm flex items-center justify-center gap-2 shadow-lg transition-all hover:scale-102">
            <Monitor className="w-4 h-4" />
            <span>Launch Zoom as Host</span>
            <ExternalLink className="w-4 h-4 ml-1" />
          </a>
          <button onClick={handleCopyLink} className="w-full sm:w-auto px-6 py-3.5 rounded-2xl bg-white/10 hover:bg-white/20 text-white text-xs sm:text-sm font-bold transition-colors flex items-center justify-center gap-2 cursor-pointer">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-5 rounded-2xl bg-slate-50 border border-slate-200">
        <div className="space-y-2">
          ...
          <div className="flex justify-between text-xs sm:text-sm font-bold text-slate-700">
            <span>{currentBatch.completedClasses} of {currentBatch.totalClasses} Classes Done</span>
        ...
        <div className="space-y-1 text-xs sm:text-sm text-slate-700 sm:pl-4 sm:border-l border-slate-200 flex flex-col justify-center">
          <p className="flex justify-between"><span className="text-slate-500 font-medium">Batch Roster:</span><strong className="text-slate-900">{currentBatch.studentsCount} Active Students</strong></p>
          <p className="flex justify-between"><span className="text-slate-500 font-medium">Class Days:</span><strong className="text-[#002b5b]">{currentBatch.schedule}</strong></p>
        </div>
      </div>
    </div>
  );
