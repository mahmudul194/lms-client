    ...
    <div className="bg-[#f8fafc] min-h-screen py-8 sm:py-14 text-slate-900 font-sans">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 space-y-8">
        <div className="flex items-center gap-2 text-xs sm:text-sm text-slate-400 font-medium">
          ...
          <Link href="/courses" className="hover:text-[#0077b6] transition-colors">Courses</Link>
          <ChevronRight className="w-3.5 h-3.5" />
        ...
        <div className="space-y-3">
          ...
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-[#002b5b] tracking-tight">Complete Your Course Enrollment</h1>
        ...
        {isSubmitted ? (
          <AdmissionSuccessSlip fullName={formData.fullName} selectedCourse={selectedCourse} batchTiming={batchTiming} paymentMethod={paymentMethod} trxId={trxId} dueToday={dueToday} />
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
            <div className="lg:col-span-7 space-y-6">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="pt-2">
                  <button type="submit" className="w-full py-4 rounded-2xl bg-[#0077b6] hover:bg-[#005a8c] text-white font-black text-base sm:text-lg shadow-xl shadow-sky-600/25 transition-all hover:scale-[1.01] active:scale-[0.99] cursor-pointer flex items-center justify-center gap-3">
                    <span>Complete Admission & Get Instant Access</span>
                    <ArrowRight className="w-5 h-5" />
                  </button>
                  <p className="text-center text-xs text-slate-500 mt-2 font-medium">By submitting, you agree to BIM Build BD Terms & Academic Policies.</p>
                </div>
              </form>
            </div>
            <div className="lg:col-span-5 sticky top-24 space-y-6">
              <AdmissionSummaryCard selectedCourse={selectedCourse} paymentType={paymentType} installmentAmount={installmentAmount} dueToday={dueToday} savings={savings} />
            </div>
          </div>
        )}
      </div>
